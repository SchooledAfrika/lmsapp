...
const page = () => {
  ...
  useEffect(() => {
    ...
    if (data?.user.role === "School") {
      router.push("http://localhost:3000/school-dashboard");
    }
    if (data?.user.role === "Teacher") {
      console.log("hello all");
      router.push("http://localhost:3000/teacher-dashboard");
    }
    if (data?.user.role === "Parents") {
      router.push("http://localhost:3000/parents-dashboard");
    }
    if (data?.user.role === "Student") {
      router.push("http://localhost:3000/student-dashboard");
    }
  }, [status, data]);
