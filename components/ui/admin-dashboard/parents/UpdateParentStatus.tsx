...
import { z } from "zod";
import { parentStatusSchema } from "@/constants/parentStatus";
...
export type IupdatingParentStatus = z.infer<typeof parentStatusSchema>;

interface Iupdate {
  dataId: string;
}
const UpdateParentStatus: React.FC<Iupdate> = ({ dataId }) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<IupdatingParentStatus>({
    resolver: zodResolver(parentStatusSchema),
  });
  ...
  const mutation = useMutation({
    mutationKey: ["updateParentStatus"],
    mutationFn: async (data: IupdatingParentStatus) => {
      // console.log(data);
    ...
    onSuccess: async (result) => {
      queryClient.invalidateQueries({ queryKey: ["updateParent"] });
      if (result.ok) {
  ...
  const runSubmit: SubmitHandler<IupdatingParentStatus> = async (data) => {
    console.log(data);
    setloading(true);
...
export default UpdateParentStatus;
