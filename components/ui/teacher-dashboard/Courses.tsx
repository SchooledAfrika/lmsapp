...
const CourseCard = ({
  ...
  return (
    <div className="w-full hover:-translate-y-2 my-6 transition-transform duration-300 group">
      <Card
        className={`${
          pricingFree ? "md:w-[320px] w-full" : "md:w-[320px] w-full opacity-45 cursor-not-allowed"
        }`}
      >
        <CardHeader>
          <Image
        ...
        <CardFooter className="flex justify-between">
          ...
          <p className={`${
          pricing ? "text-[12px] font-semibold" : "text-[12px] font-semibold"
        }`}>{pricing  || pricingFree}</p>
          </div>
