...
export interface PurchasedTeacherInfo {
  id: string;
  name: string;
  profilePhoto: string | null;
  status: string;
...
const Courses = () => {
  ...
  const { data: ICourses, isFetching, isError, error } = useQuery({
  ...
  const { data: IPurchasedCourses, isFetching, isError, error } = useQuery({
    queryKey: ["getCourseByTeacher"],
    queryFn: async () => {
